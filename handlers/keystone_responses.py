...
def get_tenants(content, parameters):
    idl = ovn_connection.connect()
    ovn_north = OvnNorth(idl)

    routers = ovn_north.list_lr()
    tenants_names = []

    for router in routers:
        if router:
            tenant = router.external_ids.get('tenant_id')
            if tenant and tenants_names.count(tenant) == 0:
                tenants_names.append(tenant)

    tenants = []

    for name in tenants_names:
        tenants.append({
            'description': name,
            'name': name,
            'id': name,
