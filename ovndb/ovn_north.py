...
class OvnNorth(object):
    ...
    def add_lr(self, name, enabled, external_ids):
        return ovn_connection.execute(
            self.idl.lr_add(router=name, may_exist=False, enabled=enabled, external_ids=external_ids)
        )
    ...
    def remove_dhcp_options(self, id, transacion=None):
        return self.idl.dhcp_options_del(id)
    ...
    def update_security_group(
        self, sec_group_id, name, description, tenant_id, transaction
    ):
        try:
            transaction.add(
                self._ovn_sec_group_api.update_security_group(
                    sec_group_id, name, tenant_id, description
                )
            )
