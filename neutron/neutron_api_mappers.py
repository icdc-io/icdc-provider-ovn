...
class NetworkMapper(Mapper):
    ...
    OVN_NETWORK_EXTERNAL_IDS = 'external_ids'
    OVN_NETWORK_TENANT_ID = 'tenant_id'
    ...
    def rest2row(wrapped_self, func, rest_network_data, network_id):
        ...
        tenant_id = rest_network_data.get(
            NetworkMapper.OVN_NETWORK_TENANT_ID
        )
        if network_id:
            return func(
                wrapped_self,
    ...
    def row2rest(network):
        ...
        tenant_id = ls.external_ids.get(NetworkMapper.OVN_NETWORK_TENANT_ID)
        result = {
            NetworkMapper.REST_NETWORK_ID: str(ls.uuid),
            NetworkMapper.REST_NETWORK_NAME: network_name or ls.name,
            NetworkMapper.OVN_NETWORK_TENANT_ID: tenant_id,
...
class SubnetMapper(Mapper):
    ...
    REST_SUBNET_TENANT_ID = 'tenant_id'
    ...
    OVN_SUBNET_TENANT_ID = 'tenant_id'
    ...
    def rest2row(wrapped_self, func, rest_data, subnet_id):
        ...
        tenant_id = rest_data.get(SubnetMapper.REST_SUBNET_TENANT_ID)
    ...
    def row2rest(row):
        ...
        result = {
            ...
            SubnetMapper.REST_TENANT_ID: row.external_ids.get('tenant_id', tenant_id()),
...
class RouterMapper(Mapper):
    ...
    # ICDC
    REST_ROUTER_EXTERNAL_IDS = 'external_ids'
    REST_ROUTER_METADATA = 'metadata'
    ...
    def rest2row(wrapped_self, func, rest_data, router_id):
        ...
        metadata = rest_data.get(RouterMapper.REST_ROUTER_METADATA, {})
        external_ids = rest_data.get(RouterMapper.REST_ROUTER_EXTERNAL_IDS, {})

        external_ids = {**external_ids, **metadata}
    ...
    def row2rest(router):
        ...
        result = {
            ...
            RouterMapper.REST_TENANT_ID: row.external_ids.get('tenant_id'),#tenant_id(),
            ...
            RouterMapper.REST_ROUTER_METADATA: RouterMapper._get_router_metadata(
                row.external_ids
            ),
        }
    ...
    @staticmethod
    def _get_router_metadata(external_ids):
        metadata_keys = ['account', 'owner', 'tenant_id']
        metadata = {}

        for metadata_key in metadata_keys:
            if external_ids.get(metadata_key):
                metadata[metadata_key] = external_ids.get(metadata_key)
...
class SecurityGroupMapper(Mapper):
    ...
    REST_SEC_GROUP_TENANT_ID = 'tenant_id'
    ...
    OVN_SECURITY_GROUP_TENANT_ID = 'tenant_id'
    ...
    optional_attr_ext_id_mapper = {
        ...
        Mapper.REST_TENANT_ID: REST_SEC_GROUP_TENANT_ID,
    ...
    def create_port_group_name(security_group_id):
        return 'ovirt_{}'.format(security_group_id.replace('-', '_'))

    @staticmethod
    def _extract_security_group_id(port_group_name):
        return port_group_name[-UUID_LENGTH:].replace('_', '-')
