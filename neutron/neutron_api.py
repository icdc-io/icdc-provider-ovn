...
class NeutronApi(object):
        ...
        with self.tx_manager.transaction() as tx:
            if localnet:
                ...
                network_id = self._create_network(
                    name, tx, mtu, port_security_enabled, tenant_id
                )
        return self._get_network(self.ovn_north.get_ls(ls_id=network_id))

    def _create_network(self, name, transaction, mtu=None, port_security=None, tenant_id=None):
        ...
        external_ids_dict[NetworkMapper.OVN_NETWORK_TENANT_ID] = tenant_id
    ...
    def _update_network_data(
        ...
        if tenant_id is not None:
            relevant_external_ids[NetworkMapper.OVN_NETWORK_TENANT_ID] = tenant_id
    ...
    def get_subnet_external_ids(
        network_id, ip_version, name, ipv6_address_mode, gateway, mtu, tenant_id
    ):
        external_ids = {
            SubnetMapper.OVN_NETWORK_ID: network_id,
            SubnetMapper.OVN_IP_VERSION: str(ip_version),
            SubnetMapper.OVN_NETWORK_TENANT_ID: tenant_id,
        }
        if name:
    ...
    def update_subnet(
        ...
        self.ovn_north.create_ovn_update_command(
            ...
            ovnconst.ROW_LSP_EXTERNAL_IDS, {SubnetMapper.OVN_SUBNET_TENANT_ID: tenant_id},
        ).execute()
    ...
    def _add_router(
        ...
        router = self.ovn_north.add_lr(name, enabled, external_ids)
    ...
    def add_router(
        ...
        return self._add_router(
            name, enabled, network_id, gateway_subnet, gateway_ip, routes, external_ids
        )
    ...
    def update_router(
        ...
        ).add(ovnconst.ROW_LR_EXTERNAL_IDS, external_ids).execute()
